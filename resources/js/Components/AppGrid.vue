...
import {computed, customRef, onMounted, Prop, ref, watch} from "vue";
    ...
    <div v-if="paginator">
        ...
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
            ...
            <table class="w-full text-sm text-left text-gray-200">
                <thead class="text-base text-gray-600 bg-slate-200 dark:text-gray-200 dark:bg-gray-700 border-b border-slate-300 dark:border-gray-600">
                <tr>
                    <!-- Columns header -->
                ...
                <tr v-for="row in paginator.data" :key="row[0]"
                    class="dark:bg-gray-800 border-b text-gray-600 dark:border-gray-700 hover:bg-slate-50 dark:hover:!bg-gray-900 dark:text-slate-400">
        ...
        <div class="flex pt-6 px-6 justify-between items-center text-xs">
            ...
            <div class="font-normal text-gray-500">
                <span>
                    {{ $t('grid.showing') }} <span class="text-black dark:text-gray-300">{{ String(paginator.from ?? '0') }}</span> {{ $t('grid.of') }} <span
                    class="text-black dark:text-gray-300">{{ String(paginator.to ?? '0') }}</span> {{ $t('grid.from') }} <span
                    class="text-black dark:text-gray-300">{{ String(paginator.total ?? '0') }}</span> {{ $t('grid.total') }}
                </span>
            </div>

            <!-- Per page select -->
            <div class="flex items-center gap-2 text-gray-500">
                <p class="">{{ $t('grid.per_page') }}</p>
                <BaseSelect
                    v-model="perPage"
                    :options="perPageOptions"
                    class="text-xs dark:text-gray-300 dark:bg-slate-800 rounded-lg border-slate-300 dark:border-gray-500"
                />
            </div>

            <!-- Paginator -->
            <div class="flex items-center border border-slate-300 dark:border-gray-500 rounded text-gray-400 text-sm">
                <button v-for="link in paginator.links" :key="link.label"
                        :class="{'bg-sky-200 text-gray-800 dark:bg-sky-400 dark:text-gray-800': link.active}"
                        :disabled="link.url === null"
                        class="px-2 py-1 disabled:text-gray-600 dark:border-gray-600 [&:not(:last-child)]:border-r disabled:bg-gray-100 disabled:text-gray-300 dark:disabled:bg-gray-600 dark:disabled:text-gray-500"
                        @click="changePage(link.url)"
                        v-html="link.label.replace('Previous', $t('grid.previous')).replace('Next', $t('grid.next'))"/>
            </div>
        </div>
    </div>
